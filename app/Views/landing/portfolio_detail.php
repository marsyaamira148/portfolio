<?php
// app/Views/landing/portfolio_detail.php
?>
    ...
    <div class="container">
      ...
      <div class="project-header" data-aos="fade-down">
        <span class="project-date">
          <?= isset($project['created_at']) ? date('j F Y', strtotime($project['created_at'])) : '-' ?>
        </span>
        <h2 class="project-title"><?= esc($project['title'] ?? '(Tanpa Judul)') ?></h2>
        <div class="header-line"></div>
      </div>

      <!-- Gambar + Info -->
      <div class="project-top-box" data-aos="zoom-in">
        <div class="project-image-box" data-aos="fade-right">
          <img src="<?= base_url('uploads/' . ($project['thumbnail'] ?? 'default.png')) ?>"
               class="project-image"
               alt="<?= esc($project['title'] ?? 'Default Thumbnail') ?>">
        </div>

        <div class="project-info-box" data-aos="fade-left">
          <h5 class="info-title"><i class="bi bi-info-circle"></i> Informasi Proyek</h5>
          <div class="info-item"><h6>Client</h6><p><?= esc($project['client'] ?? '-') ?></p></div>
          <div class="info-item"><h6>Kategori</h6><p><?= esc($project['category'] ?? '-') ?></p></div>
          <div class="info-item"><h6>Tools</h6><p><?= esc($project['tools'] ?? '-') ?></p></div>
          <div class="info-item"><h6>Start Date</h6><p><?= !empty($project['start_date']) ? date('j F Y', strtotime($project['start_date'])) : '-' ?></p></div>
          <div class="info-item"><h6>End Date</h6><p><?= !empty($project['end_date']) ? date('j F Y', strtotime($project['end_date'])) : '-' ?></p></div>

          <?php if(!empty($project['project_file'])): ?>
          <div class="info-item" style="justify-content:flex-start;">
            <h6 style="margin-right:10px;">Project File</h6>
            <a href="<?= base_url('uploads/' . $project['project_file']) ?>" target="_blank" class="btn btn-file">
      ...
      <!-- Deskripsi -->
      <div class="project-desc-box" data-aos="fade-up">
        <h5>Deskripsi Proyek</h5>
        <p><?= $project['description'] ?? '-' ?></p>
      </div>

      <!-- ================= POPULAR PORTFOLIO / POSTER ================= -->
      <section class="popular-portfolio mt-5">
        <div class="container">
          <h1 class="fw-bold text-center mb-4" data-aos="fade-up">Portfolio Lainnya</h1>
          <div class="row justify-content-center">
            <?php foreach($popular_portfolio as $p): ?>
            <div class="col-md-4 mb-4" data-aos="fade-up" data-aos-delay="100">
              <a href="<?= base_url('portfolio/' . $p['slug']) ?>" class="text-decoration-none">
                <div class="popular-card shadow-sm">
                  <div class="popular-img-box">
                    <img src="<?= base_url('uploads/' . $p['thumbnail']) ?>" class="popular-img" alt="<?= esc($p['title']) ?>">
                  </div>
